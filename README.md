...
As releases já incluem o relay público padrão do Railway pelo TCP Proxy IPv4, então uma instalação nova tenta conectar automaticamente e refaz a tentativa a cada 15 segundos quando a reserva cai. Para trocar o relay em uma nova compilação, defina `TEAMSCORD_DEFAULT_RELAY_ADDRESS`; o painel de rede continua permitindo adicionar relays extras ou usar conexão direta/LAN.
